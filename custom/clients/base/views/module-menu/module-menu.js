...
({
    ...
    handleRouteEvent: function(event) {
        var currentRoute, $currentTarget = this.$(event.currentTarget),
        ...
        if(route == '#Cotizador')//added the if condition to check my element loaded to view
        {
            app.router.navigate('#Home/layout/cotizador-lysi', {
                trigger: true
            });
        ...
        {//defaultsugar code
            (currentRoute === route) ? app.router.refresh(): app.router.navigate(route, {
                trigger: true
            });
